...
const int MAXN=5e4+5,MAXM=1e5+6e4+5,INF=~0U>>1;
...
int calF(int u,int T,int f)
{
	...
	for(int & i=G[u];i;i=e[i].next)
	{
		...
		{
			int res=calF(v,T,min(f,e[i].cap));
			if(res>0)
			{
...
int main()
{
	...
	if(mustFlow==flowSum)
	{
		int feasibleFlow=e[G[t]^1].cap;
		e[G[t]].cap=e[G[t]^1].cap=0;
		int minusFlow=dinic(t,s);
		int ans=feasibleFlow-minusFlow;
		printf("%d\n",ans>0?ans:0);
