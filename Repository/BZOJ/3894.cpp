...
int main()
{
    ...
    for(i=1;i<=n;i++) for(j=1;j<=m;j++)
    {
        ...
        if(i>1) addEdge(ncnt,id[i-1][j],INF);
        if(i<n) addEdge(ncnt,id[i+1][j],INF);
        if(j>1) addEdge(ncnt,id[i][j-1],INF);
        if(j<m) addEdge(ncnt,id[i][j+1],INF);
        addEdge(S,ncnt,x);
    }
    for(i=1;i<=n;i++) for(j=1;j<=m;j++)
    {
        ...
        if(i>1) addEdge(id[i-1][j],ncnt,INF);
        if(i<n) addEdge(id[i+1][j],ncnt,INF);
        if(j>1) addEdge(id[i][j-1],ncnt,INF);
        if(j<m) addEdge(id[i][j+1],ncnt,INF);
        addEdge(ncnt,T,x);
    }
