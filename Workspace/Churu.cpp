...
const int MAXN=1e5+5,SN=355,MAXQ=1e5+5;
...
ull calSum(int x) {return sum1[(x-1)/bsz+1]+sum2[(x-1)/bsz+1][x];}
...
void mdf(int p,int v)
{
	...
	for(i=1;i<=bnum;i++) sum[i]+=(v-a[p])*cont[i][p];
...
ull qry(int l,int r)
{
	...
	else
	{
		...
		for(i=(rB-1)*bsz+1;i<=rB;i++) res+=calSum(fl[i],fr[i]);
	}
	return res;
}
